...
int main()
{
    ...
    printf("Integracion Numerica (Suma de Riemann) - version paralela\n");
    ...
    // Iteraciones independientes: reduction(+:) combina los resultados sin critical/atomic
    #pragma omp parallel for reduction(+:areaTotal) schedule(static) private(xi, altura, areaRectangulo)
    for (i = 0; i < n; i++)
    {
        // Calcular xi
        xi = a + i * dx;
